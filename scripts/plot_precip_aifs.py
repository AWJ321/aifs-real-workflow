#!/usr/bin/env python3
"""
AIFS precipitation plot — log colorscale.
...
def plot_frame(base_name, step, init_str):
    fname      = f"{base_name}-out-{step}.nc"
    fname_prev = f"{base_name}-out-{step-6}.nc"
    fpath      = os.path.join(PROCESSED_DIR, fname)
    fpath_prev = os.path.join(PROCESSED_DIR, fname_prev)

    if not os.path.exists(fpath):
        return None

    ds  = subset(open_nc(fpath))
    tp  = ds["tp"].squeeze().values

    if os.path.exists(fpath_prev):
        ds_prev = subset(open_nc(fpath_prev))
        tp_prev = ds_prev["tp"].squeeze().values
        rain_6h = np.maximum(tp - tp_prev, 0)
        ds_prev.close()
    else:
        rain_6h = np.maximum(tp, 0)

    lat = ds["lat"].values
    lon = ds["lon"].values
    ds.close()

    rain_6h = gaussian_filter(rain_6h, sigma=SIGMA_RAIN)

    fig, ax = plt.subplots(1, 1, figsize=(10, 8),
                           subplot_kw={"projection": ccrs.PlateCarree()})
    base(ax, f"AIFS Precip | Init {init_str} | Lead +{step}h")

    rain_plot = np.ma.masked_less(rain_6h, 0)
    cf = ax.contourf(
        lon, lat, rain_plot,
    ...
    fig.suptitle("AIFS Forecast — Precipitation", fontsize=13, fontweight="bold")
...
def main():
    ...
    gif_path   = os.path.join(GIF_DIR, f"aifs_precip_{base_name}.gif")

    os.makedirs(GIF_DIR, exist_ok=True)
    frames_dir = os.path.join(FRAMES_DIR, base_name)
    os.makedirs(frames_dir, exist_ok=True)

    print("=" * 60)
    print(" AIFS Precipitation Plot")
    print(f" Cycle: {init_time}")
    print("=" * 60)

    if os.path.exists(gif_path):
        print(f"GIF already exists for {base_name}, skipping.")
        return
    ...
    for step in steps:
        ...
        fig = plot_frame(base_name, step, init_str)
        if fig is None:
            print(f"  Missing file for step {step}, skipping")
            continue

        png_path = os.path.join(frames_dir,
                                f"aifs_precip_{base_name}-lead-{step:03d}h.png")
    ...
    print("=" * 60)
    print(" AIFS Precipitation plot complete!")
    print("=" * 60)
